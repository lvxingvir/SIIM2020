...
def dicom2png_512(dicom_filepath, png_filepath=None, size = 512,normalize_functor=scale_to_255, dryrun=False):
    """Convert dicom image to png file
    ...
    if image_array.max() <= 1:
        image_array = (image_array * 255).astype(np.uint8)
    if not dryrun:
        image_array = cv2.resize(image_array, (size,size), cv2.INTER_AREA)
        fileio.maybe_make_new_dir(os.path.dirname(png_filepath))
        cv2.imwrite(png_filepath, image_array)
    return image_array
        
def dicom2png_512_batch(dicom_search_path, png_filepath_replacer, size=512, nfiles=None, normalize_functor=scale_to_255):
    """Convert png to nii in batch mode
    ...
    for dicom_filepath in tqdm(file_list):
        png_filepath = png_filepath_replacer(dicom_filepath)
        print('{} --> {}'.format(dicom_filepath, png_filepath))
        dicom2png_512(dicom_filepath, png_filepath, size = size,normalize_functor=normalize_functor)
